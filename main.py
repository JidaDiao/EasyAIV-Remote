...
class ModelClientProcess(Process):
    def __init__(self, input_image, device, model_process_args):
        ...
        ################# 远程修改
        host = '192.168.50.13'  # 服务器地址, 替换成你实际的服务器IP
        port = 11453  # 服务器端口
        self.client_socket = socket.socket()  # 创建socket对象
        self.client_socket.connect((host, port))  # 连接到服务器
        #################
    ...
    def run(self):
        ...
        while True:
            ...
            if cached is not None and hit_in_a_row < self.model_fps_number.value:
                ...
                ###################
                if not self.input_image_q.empty():
                    data = (input_image, mouth_eye_vector, pose_vector, eyebrow_vector, mouth_eye_vector_c,
                            eyebrow_vector_c, 1)
                else:
                    data = (input_image, mouth_eye_vector, pose_vector, eyebrow_vector, mouth_eye_vector_c,
                            eyebrow_vector_c, 0)
                ...
                data_len = len(serialized_data).to_bytes(4, 'big')
                self.client_socket.sendall(data_len + serialized_data) # 发送数据到服务器
                # 接收服务器返回的模型输出
                response_len = self.client_socket.recv(4)
                response_len = int.from_bytes(response_len, 'big')
                response = b''
                while len(response) < response_len:
                    chunk = self.client_socket.recv(response_len - len(response))
                    if not chunk:
                        break
                    response += chunk
                # 反序列化输出
                output_image = pickle.loads(response)
